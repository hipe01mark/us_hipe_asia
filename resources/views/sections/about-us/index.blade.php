<section id="about" class="s-about">
    <div class="l-wrapper l-wrapper--fluid l-wrapper--padded-y">
        <div class="s-about__container">
            <div class="s-about__container-left">
                <div class="image--bg">
                    <img src="{{ mix('assets/images/about/about_bg.webp') }}" alt="about-img" width="100%" height="100%">
                </div>
            </div>
            <div class="s-about__container-right">
                <div class="content">
                    <h2>Who we are</h2>
                    <h3>Experience the essence of HiPE</h3>
                    <p>HiPE is your trusted partner for innovative IT solutions. Our team of skilled Filipino engineers
                        excels in web and mobile development, design, and maintenance. We're committed to bringing your
                        vision to life and fostering lasting partnerships built on trust and excellence.</p>
                </div>
                <div class="image">
                    <img src="{{ mix('assets/images/about/about_img.webp') }}" alt="about-hipe" class="card-img"
                        width="100%" height="100%">
                </div>
            </div>
        </div>
    </div>
</section>
